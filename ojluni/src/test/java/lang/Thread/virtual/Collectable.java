...
public class Collectable {
    ...
    public void testUnstartedThread() {
        Utils.assumeVirtualThreadFlagTrue();
        //var thread = Thread.ofVirtual().unstarted(() -> { });
        //var ref = new WeakReference<>(thread);
        //thread = null;
        var ref = getUnstartedThread();
        waitUntilCleared(ref);
    }

    private static WeakReference<Thread> getUnstartedThread() {
        var thread = Thread.ofVirtual().unstarted(() -> { });
    ...
    public void testTerminatedThread() throws Exception {
        ...
        //var ref = new WeakReference<>(thread);
        //thread = null;
        var ref = getTerminatedThread();
        waitUntilCleared(ref);
    }

    private static WeakReference<Thread> getTerminatedThread() {
        var thread = Thread.ofVirtual().unstarted(() -> { });
        return new WeakReference<>(thread);
    }

    private static void waitUntilCleared(WeakReference<?> ref) {
        // Android-changed: Both GC and finalization are needed, or it could fail in the gcstress.
        /*
        while (ref.get() != null) {
            System.gc();
            try {

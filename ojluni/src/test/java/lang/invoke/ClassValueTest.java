 ...
 * @author jrose
 */
public final class ClassValueTest {
    ...
    public void testRemoveStale() throws InterruptedException {
        ...
        assertEquals("input not updated", 1, input.get());
        assertEquals("CV not using up-to-date input", 1, (int) cv.get(int.class));
    }
     ...
     */
    @Test
    public void testRemoveInCompute() {
    ...
    */
    ...
    @Ignore
    public void testWeakAgainstClassValue() {
        ClassValue<int[]> cv = new ClassValue<>() {
            @Override
            protected int[] computeValue(Class<?> type) {
                return new int[23];
    ...
    // Android-removed: ScopedValue-s are not supported.
    /*
    private static final ScopedValue<Integer> THREAD_ID = ScopedValue.newInstance();

    @Test
    public void testNoRecomputeOnUnrelatedRemoval() throws InterruptedException {
        ...
        awaitThreads(t1, t2);
        assertEquals("Redundant computeValue retries", 1, t1Tries.get());
    }

    @Test
    public void testNoObsoleteInstallation() throws InterruptedException {
        ...
        var t = Thread.startVirtualThread(() -> ScopedValue.where(THREAD_ID, 1).run(() -> {
            ...
            assertEquals("recomputed value", 42, v);
        }));
        await(slowComputationStart);
        assertEquals("fast computation installed value", 5, (int) clv.get(int.class));
        clv.remove(int.class);
        slowComputationContinue.countDown();
        awaitThreads(t);
        assertEquals("slow computation reinstalled value", 42, (int) clv.get(int.class));
    }
    */
}
